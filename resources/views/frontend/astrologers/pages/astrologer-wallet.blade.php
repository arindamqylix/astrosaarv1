        ...
        <div class="breadcrumb-section d-none d-md-block">
            <div class="container">
                <div class="breadcrumbs">
                    ...
                    <a href="{{ route('front.getAstrologerWallet') }}">My Wallet</a>
                </div>
            </div>
        </div>
                        ...
                        <form method="post" id="editwithdrawForm">
                            ...
                            <div class="form-group-modern">
                                ...
                                <div class="radio-group">
                                    @foreach($withdrawMethod as $method)
                                        <div class="radio-option">
                                            <input type="radio" name="paymentMethod" value="{{ $method->method_id }}" id="edit_method_{{ $method->id }}" class="method-radio" data-method-id="{{ $method->method_id }}" required>
                                            <label for="edit_method_{{ $method->id }}">{{ $method->method_name }}</label>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
            ...
            <div class="wallet-card">
                <div class="balance-section">
                    <div class="balance-item current">
                        ...
                        <div class="balance-amount">{{ $currency['value'] }}{{ number_format($getProfile['data']['totalWalletAmount'], 2) }}</div>
                    ...
                    <div class="balance-item earning">
                        ...
                        <div class="balance-amount">{{ $currency['value'] }}{{ number_format($withdrawlrequest['recordList']['totalEarning'], 2) }}</div>
                    ...
                    <div class="balance-item withdrawal">
                        ...
                        <div class="balance-amount">{{ $currency['value'] }}{{ number_format($withdrawlrequest['recordList']['withdrawAmount'], 2) }}</div>
                    ...
                    <div class="balance-item pending">
                        ...
                        <div class="balance-amount">{{ $currency['value'] }}{{ number_format($withdrawlrequest['recordList']['totalPending'], 2) }}</div>
                ...
                <div class="wallet-actions">
                    ...
                    <a href="{{ route('front.AstrologerWalletRecharge') }}" class="btn-wallet btn-add-money" id="btnAddMoney">
                    ...
                    <div class="wallet-card">
                        ...
                        <div class="table-container">
                            <table class="table-modern">
                                ...
                                <tbody>
                                    @forelse ($getAstrologer['recordList'][0]['payment'] as $walletdata)
                                        @if (!empty($walletdata))
                                            <tr>
                                                <td>{{ $walletdata['orderId'] }}</td>
                                                <td>{{ $walletdata['paymentMode'] }}</td>
                                                <td><strong>{{ $currency['value'] }}{{ number_format($walletdata['amount'], 2) }}</strong></td>
                                                <td>{{ $currency['value'] }}{{ number_format($walletdata['cashback_amount'], 2) }}</td>
                                                <td>
                                                    @if ($walletdata['paymentStatus'] == 'success')
                                                        <span class="status-badge status-success">{{ $walletdata['paymentStatus'] }}</span>
                                                    @else
                                                        <span class="status-badge status-danger">{{ $walletdata['paymentStatus'] }}</span>
                                                    @endif
                                                </td>
                    ...
                    <div class="wallet-card">
                        ...
                        <div class="table-container">
                            <table class="table-modern">
                                ...
                                <tbody>
                                    @forelse ($getAstrologer['recordList'][0]['wallet'] as $walletdata)
                                        @if (!empty($walletdata))
                                            <tr>
                                                <td>
                                                    <div style="font-weight: 600; color: #333; margin-bottom: 5px;">
                                                        @if ($walletdata['transactionType'] == 'Call' || $walletdata['transactionType'] == 'Chat')
                                                            ...
                                                            {{ $walletdata['transactionType'] }} with {{ $walletdata['name'] }} for {{ $walletdata['totalMin'] }} minutes
                                                        ...
                                                        @elseif($walletdata['transactionType'] == 'Gift')
                                                            ...
                                                            Received {{ $walletdata['transactionType'] }} From {{ $walletdata['name'] }}
                                                        @else
                                                ...
                                                <td>
                                                    @if ($walletdata['transactionType'] == 'Cashback' || ($walletdata['transactionType'] == 'astromallOrder' && $walletdata['isCredit']==1) || $walletdata['transactionType'] != 'KundliView')
                                                        <div style="color: #28a745; font-weight: 600; font-size: 16px;">
                                                            +{{$currency['value']}}{{ number_format($walletdata['amount'], 2) }}
                                                        </div>
                    ...
                    <div class="wallet-card">
                        ...
                        <div class="table-container">
                            <table class="table-modern">
                                ...
                                <tbody>
                                    @forelse ($withdrawlrequest['recordList']['withdrawl'] as $withdrawl)
                                        @if (!empty($withdrawl))
                                            <tr>
                                                <td>
                                                    <strong style="font-size: 16px; color: #333;">
                                                        {{ $currency['value'] }}{{ number_format($withdrawl['withdrawAmount'], 2) }}
                                                    </strong>
                                                </td>
                                                <td>{{ date('d M Y, h:i A', strtotime($withdrawl['created_at'])) }}</td>
                                                <td>
                                                    @if ($withdrawl['status'] == 'Pending')
                                                        <span class="status-badge status-warning">{{ $withdrawl['status'] }}</span>
                                                    @else
                                                        <span class="status-badge status-success">{{ $withdrawl['status'] }}</span>
                                                    @endif
                                                </td>
                                                <td class="text-center">
                                                    @if($withdrawl['status'] == 'Pending')
                                                        <a data-toggle="modal" data-target="#editwithdraw" class="btn-edit editwithdraw" data-field='@json($withdrawl)'>
