                    ...
                    <div class="mt-4">
                        ...
                        <div class="table-container">
                            ...
                            <tbody>
                                    ...
                                    @if(isset($getUserById['recordList'][0]['orders']['order']) && count($getUserById['recordList'][0]['orders']['order']) > 0)
                                        @foreach($getUserById['recordList'][0]['orders']['order'] as $orderdata)
                                            @if(!empty($orderdata))
                                                <tr>
                                                    <td>
                                                        @if(isset($orderdata['invoice_link']) && !empty($orderdata['invoice_link']))
                                                            <a href="{{ $orderdata['invoice_link'] }}" class="btn-invoice" target="_blank">
                                                    ...
                                                    <td>
                                                        <span class="product-name">{{ $orderdata['productName'] ?? 'N/A' }}</span>
                                                    </td>
                                                    <td>
                                                        @if(isset($orderdata['productImage']) && !empty($orderdata['productImage']))
                                                            <img src="/{{ $orderdata['productImage'] }}" alt="{{ $orderdata['productName'] ?? 'Product' }}" class="product-image">
                                                        @else
                                                            <span style="color: #999;">-</span>
                                                        @endif
                                                    </td>
                                                    <td>
                                                        <strong>{{ $currency['value'] ?? '₹' }}{{ number_format($orderdata['productAmount'] ?? 0, 2) }}</strong>
                                                    </td>
                                                    <td>{{ $orderdata['gstPercent'] ?? 0 }}%</td>
                                                    <td>
                                                        <strong style="color: #6d4ec6;">{{ $currency['value'] ?? '₹' }}{{ number_format($orderdata['totalPayable'] ?? 0, 2) }}</strong>
                                                    </td>
                                                    <td>
                                                        @if(isset($orderdata['orderStatus']))
                                                            ...
                                                            @elseif($orderdata['orderStatus'] == 'Cancelled')
                                                                <span class="status-badge status-cancelled">{{ $orderdata['orderStatus'] }}</span>
                                                            @elseif($orderdata['orderStatus'] == 'Delivered')
                                                                <span class="status-badge status-delivered">{{ $orderdata['orderStatus'] }}</span>
                                                            @else
                                                                <span class="status-badge status-completed">{{ $orderdata['orderStatus'] }}</span>
                                                            @endif
                                                        @else
                                                            <span style="color: #999;">-</span>
                                                        @endif
                                                    ...
                                                    <td>
                                                        {{ isset($orderdata['created_at']) ? date('d M Y, h:i A', strtotime($orderdata['created_at'])) : 'N/A' }}
                                                    </td>
                                                    <td>
                                                        @if(isset($orderdata['orderStatus']) && $orderdata['orderStatus'] != 'Cancelled' && $orderdata['orderStatus'] != 'Delivered')
