                ...
                <div class="col-md-12 d-flex align-items-center">
                    <span style="text-transform: capitalize;">
                        <span class="text-white breadcrumbs" style="font-size: 14px;">
                            ...
                            <a href="{{ route('front.getMyWallet') }}" style="color:white;text-decoration:none; transition: color 0.3s ease;" onmouseover="this.style.color='#e5b86a'" onmouseout="this.style.color='white'">My Wallet</a>
                        </span>
                    </span>
                </div>
            </div>
                ...
                <div class="col-lg-8">
                    <div class="recharge-card">
                        ...
                        <div class="balance-display">
                            <span>Available Balance: <strong style="color: #6d4ec6;">{{ $currency['value'] }}{{ number_format($getProfile['data']['totalWalletAmount'], 2) }}</strong></span>
                        </div>

                        <!-- Recharge Amount Grid -->
                        <div class="recharge-grid" id="plan-item">
                            @foreach ($getRechargeAmount['recordList'] as $index => $amount)
                                <div class="recharge-card-item add-plan" data-amount="{{ $amount['amount'] }}" data-cashback="{{ $amount['cashback'] }}" 
                                     style="{{ $index == 0 ? 'border-color: #6d4ec6; border-width: 3px;' : '' }}">
                                    <div class="recharge-amount">
                                        <p class="recharge-amount-value">{{ $currency['value'] }}{{ number_format($amount['amount'], 0) }}</p>
                                    </div>
                                    <div class="recharge-badge">
                                        {{ $amount['cashback'] }}% Extra
                                    </div>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="order-summary-box">
                        ...
                        <form id="prcdtopayform">
                            ...
                            <div class="order-row">
                                ...
                                <span class="order-value" id="total_plan_amount">{{ $currency['value'] }}{{ number_format($selectedamount['amount'], 2) }}</span>
                            </div>
                            ...
                            <div class="order-row" id="gst_details">
                                <span class="order-label">
                                    <span id="gstval">{{ $gstvalue['value'] }}</span>% GST(+)
                                </span>
                                <span class="order-value" id="total_plan_gst_amount">{{ $currency['value'] }}{{ number_format($selectedamount['amount'] * ($gstvalue['value'] / 100), 2) }}</span>
                            </div>

                            <div class="order-row">
                                <span class="order-label order-total">Total</span>
                                <span class="order-value order-total total_plan_amount_with_gst">{{ $currency['value'] }}{{ number_format($selectedamount['amount'] + $selectedamount['amount'] * ($gstvalue['value'] / 100), 2) }}</span>
                            </div>
                            ...
                            <div class="cashback-info" id="total-cashback-amount">
                                ...
                                <div class="cashback-text">
                                    <div class="cashback-percent" id="total_plan_cashback_percent">{{ $selectedamount['cashback'] }}% Extra</div>
                                    <div class="cashback-amount" id="total_plan_cashback_amount">{{ $currency['value'] }}{{ number_format(($selectedamount['amount'] * $selectedamount['cashback']) / 100, 2) }} cashback</div>
                                </div>
                            </div>
                            ...
                            <button type="button" class="btn-proceed" id="proceedbtn">
                                Proceed To Pay {{ $currency['value'] }}{{ number_format($selectedamount['amount'] + $selectedamount['amount'] * ($gstvalue['value'] / 100), 2) }}
                            </button>
