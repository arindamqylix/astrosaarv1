                    ...
                    <div class="balance-section">
                        <div class="d-flex align-items-center justify-content-between flex-wrap">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <h3 class="balance-amount" id="wallbalance">
                                        {{ $currency['value'] }}{{ number_format($getProfile['data']['totalWalletAmount'], 2) }}
                                    </h3>
                            ...
                            <div class="mt-3 mt-md-0">
                                ...
                                <a href="{{ route('front.walletRecharge') }}" class="btn-add-money" id="btnAddMoney">
                                    Add Money
                                </a>
                            </div>
                        </div>
                        ...
                        <div class="table-responsive tab-pane fade show active" id="paymentLogsTable">
                            ...
                            <div class="table-container">
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        @if(isset($getUserById['recordList'][0]['paymentLogs']['payment']) && count($getUserById['recordList'][0]['paymentLogs']['payment']) > 0)
                                            @foreach ($getUserById['recordList'][0]['paymentLogs']['payment'] as $walletdata)
                                                @if (!empty($walletdata))
                                                    <tr>
                                                        <td><strong>{{ $walletdata['orderId'] ?? 'N/A' }}</strong></td>
                                                        <td>{{ $walletdata['paymentMode'] ?? 'N/A' }}</td>
                                                        <td><strong>{{ $currency['value'] }}{{ number_format($walletdata['amount'] ?? 0, 2) }}</strong></td>
                                                        <td>{{ $currency['value'] }}{{ number_format($walletdata['cashback_amount'] ?? 0, 2) }}</td>
                                                        <td>
                                                            @if (isset($walletdata['paymentStatus']) && $walletdata['paymentStatus'] == 'success')
                                                                <span class="status-badge status-success">Success</span>
                                                            @else
                                                                <span class="status-badge status-failed">{{ $walletdata['paymentStatus'] ?? 'Failed' }}</span>
                                                            @endif
                                                        </td>
                                                        <td>{{ isset($walletdata['created_at']) ? date('d M Y, h:i A', strtotime($walletdata['created_at'])) : 'N/A' }}</td>
                                                    </tr>
                                                @endif
                                            @endforeach
                                        @else
                        ...
                        <div class="table-responsive tab-pane fade" id="walletTransactionTable">
                            ...
                            <div class="table-container">
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        @if(isset($getUserById['recordList'][0]['walletTransaction']['wallet']) && count($getUserById['recordList'][0]['walletTransaction']['wallet']) > 0)
                                                        ...
                                                        <td>
                                                            <div class="transaction-type">
                                                                @if ($walletdata['transactionType'] == 'Call' || $walletdata['transactionType'] == 'Chat')
                                                                    {{ $walletdata['transactionType'] }} with {{ $walletdata['name'] ?? 'Astrologer' }} for {{ $walletdata['totalMin'] ?? 0 }} minutes
                                                                @elseif($walletdata['transactionType'] == 'astromallOrder' && isset($walletdata['isCredit']) && $walletdata['isCredit']==0)
                                                                    Product Ordered
                                                                @elseif($walletdata['transactionType'] == 'astromallOrder' && isset($walletdata['isCredit']) && $walletdata['isCredit']==1)
                                                                    Product Cancelled
                                                                @elseif($walletdata['transactionType'] == 'Gift')
                                                                    Sent {{ $walletdata['transactionType'] }} to {{ $walletdata['name'] ?? 'Astrologer' }}
                                                                @else
                                                                    {{ $walletdata['transactionType'] }} Received
                                                                @endif
                                                            </div>
                                                            <div class="transaction-date">
                                                                {{ isset($walletdata['created_at']) ? date('j F Y, h:i A', strtotime($walletdata['created_at'])) : 'N/A' }}
                                                            </div>
                                                        </td>
                                                        <td>
                                                            @if ($walletdata['transactionType'] == 'Cashback' || ($walletdata['transactionType'] == 'astromallOrder' && isset($walletdata['isCredit']) && $walletdata['isCredit']==1))
                                                                <div class="amount-positive">+{{ $currency['value'] }}{{ number_format($walletdata['amount'] ?? 0, 2) }}</div>
                                                            @else
                                                                <div class="amount-negative">-{{ $currency['value'] }}{{ number_format($walletdata['amount'] ?? 0, 2) }}</div>
                                                            @endif
                                                        </td>
                                                    </tr>
                                                @endif
