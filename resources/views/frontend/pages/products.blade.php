                    ...
                    <div class="search-filter-content">
                        <div class="search-box-modern">
                            <form action="{{ route('front.getproducts') }}" method="GET">
                                @if(isset($productCategoryId) && $productCategoryId != '0')
                                    <input type="hidden" name="productCategoryId" value="{{ $productCategoryId }}">
                                @endif
                                <input value="{{ isset($searchTerm) ? $searchTerm : '' }}" name="s" placeholder="Search products by name..." type="search" autocomplete="off">
                                <button type="submit">
                        ...
                        <div>
                            <select name="productCategoryId" onchange="onFilterProductCategoryList()" class="filter-select-modern" id="psychicCategories">
                                <option value="0" {{ $productCategoryId == '0' ? 'selected' : '' }}>All Categories</option>
            ...
            <div class="products-grid">
                @foreach ($getAstromallProduct['recordList'] as $shop)
                    <div class="product-card">
                        <a href="{{ route('front.getproductDetails', ['id' => $shop['id']]) }}" class="product-image-container">
                            @php
                        ...
                        <div class="product-card-body">
                            ...
                            <div class="product-price">
                                <span class="product-price-currency">{{$currency['value']}}</span>{{ number_format($shop['amount'], 2) }}
                            </div>
            ...
            @if (isset($totalRecords) && $totalRecords > 0)
                <div class="pagination-section">
                    <div class="pagination-info">
                        Showing page {{ $currentPage }} of {{ ceil($totalRecords / $perPage) }} ({{ $totalRecords }} products)
                    </div>
                    <div class="pagination-buttons">
                        @if ($currentPage > 1)
                            <a class="btn-pagination" href="{{ route('front.getproducts', array_merge(request()->except('page'), ['page' => $currentPage - 1])) }}">
