                    ...
                    <div class="mt-4">
                        ...
                        <div class="table-container">
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    @if(isset($getUserById['recordList'][0]['reportRequest']['reportHistory']) && count($getUserById['recordList'][0]['reportRequest']['reportHistory']) > 0)
                                            ...
                                            @if (!empty($reportdata))
                                                <tr>
                                                    <td>
                                                        <div class="report-details">
                                                            <h5 class="report-title">
                                                                Asked For Report to {{ $reportdata['astrologerName'] ?? 'Astrologer' }}
                                                            </h5>
                                                            <div class="report-date">
                                                                {{ isset($reportdata['created_at']) ? date('j F Y, h:i A', strtotime($reportdata['created_at'])) : 'N/A' }}
                                                            </div>
                                                            <div>
                                                    ...
                                                    <td class="deduction-amount">
                                                        -{{ $currency['value'] }}{{ number_format($reportdata['reportRate'] ?? 0, 2) }}
                                                    </td>
                                                    <td style="text-align: right;">
                                                        @if(isset($reportdata['reportFile']) && !empty($reportdata['reportFile']))
                                                            <a href="/{{ $reportdata['reportFile'] }}" class="btn-view-report" target="_blank">
                                                                View Report
                                                            </a>
                                                        @else
